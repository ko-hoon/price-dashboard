...
function createDashboardMetalChart(data) {
    ...
    const labels = data.map(row => formatDate(row['날짜']).slice(0, 7)); // YYYY-MM만 표시
    ...
    recentData.forEach(row => {
        html += `
            <tr class="hover:bg-gray-50">
                <td class="px-4 py-3 text-sm text-gray-900">${formatDate(row['날짜'])}</td>
                <td class="px-4 py-3 text-sm text-right text-gray-700">${formatNumber(row['철근 (천원/톤)'])}</td>
                <td class="px-4 py-3 text-sm text-right text-gray-700">${formatNumber(row['철광석 (달러/톤)'])}</td>
                <td class="px-4 py-3 text-sm text-right text-gray-700">${formatNumber(row['유연탄 (달러/톤)'])}</td>
                <td class="px-4 py-3 text-sm text-right text-gray-700">${formatNumber(row['스크랩 (달러/톤)'])}</td>
            </tr>
        `;
    });
...
function createNonMetalTable(data) {
    ...
    recentData.forEach(row => {
        html += `
            <tr class="hover:bg-gray-50">
                <td class="px-4 py-3 text-sm text-gray-900">${formatDate(row['일자'])}</td>
                <td class="px-4 py-3 text-sm text-right text-gray-700">${formatNumber(row['구리 (달러/톤)'])}</td>
                <td class="px-4 py-3 text-sm text-right text-gray-700">${formatNumber(row['알루미늄 (달러/톤)'])}</td>
                <td class="px-4 py-3 text-sm text-right text-gray-700">${formatNumber(row['아연 (달러/톤)'])}</td>
                <td class="px-4 py-3 text-sm text-right text-gray-700">${formatNumber(row['납 (달러/톤)'])}</td>
                <td class="px-4 py-3 text-sm text-right text-gray-700">${formatNumber(row['니켈 (달러/톤)'])}</td>
                <td class="px-4 py-3 text-sm text-right text-gray-700">${formatNumber(row['주석 (달러/톤)'])}</td>
            </tr>
        `;
    });
...
function createOilTable(data) {
    ...
    const regions = ['전국', '서울', '경기', '인천', '강원', '충북', '충남', '전북', '전남', 
                     '경북', '경남', '세종', '대전', '대구', '부산', '광주', '울산', '제주'];
    ...
    regions.forEach(region => {
        ...
        html += `
            <div class="bg-white p-4 rounded-lg border border-gray-200">
                ...
                <div class="text-xl font-bold text-gray-900">${formatNumber(price)} 원</div>
            </div>
        `;
    });
    
    html += `</div><div class="mt-4 text-sm text-gray-500">최종 업데이트: ${formatDate(latestData['일자'])}</div>`;
...
function createExchangeTable(data) {
    ...
    recentData.forEach(row => {
        html += `
            <tr class="hover:bg-gray-50">
                <td class="px-4 py-3 text-sm text-gray-900">${formatDate(row['일자'])}</td>
                <td class="px-4 py-3 text-sm text-right text-gray-700">${formatNumber(row['환율'])}</td>
                <td class="px-4 py-3 text-sm text-right text-gray-700">${row['금리']}%</td>
                <td class="px-4 py-3 text-sm text-right text-gray-700">${row['KORIBOR']}%</td>
            </tr>
        `;
    });
