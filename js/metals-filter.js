...
class MetalsPageFilter {
    ...
    async init() {
        try {
            ...
            // 데이터 로드
            console.log("철강 데이터 로딩 시작...");
            this.dataCache = await loadCSV("monthly_metal_data.csv");
            ...
            // 날짜 필터 초기화
            this.dateFilter = new DateRangeFilter("metal-date-filter", {
                defaultRange: "ALL",
    ...
    updateDisplay(dateRange) {
        ...
        // 차트 업데이트
        if (typeof createMetalChart === "function") {
            createMetalChart(this.filteredData);
        }

        // 테이블 업데이트
        if (typeof createMetalTable === "function") {
            createMetalTable(this.filteredData);
        }
    }

    // Excel 다운로드 이벤트 연결
    attachDownloadEvent() {
        const downloadBtn = document.getElementById("download-metal-excel");
        if (downloadBtn) {
    ...
    downloadExcel() {
        ...
        const preparedData = this.filteredData.map((row) => {
            return {
                날짜: formatDate(row["날짜"]),
                "철근 (천원/톤)": parseFloat(row["철근 (천원/톤)"]) || 0,
                "철광석 (달러/톤)": parseFloat(row["철광석 (달러/톤)"]) || 0,
                "유연탄 (달러/톤)": parseFloat(row["유연탄 (달러/톤)"]) || 0,
                "스크랩 (달러/톤)": parseFloat(row["스크랩 (달러/톤)"]) || 0,
            };
        });
...
// 페이지 로드 시 자동 실행
let metalsPageFilter = null;

window.addEventListener("DOMContentLoaded", async () => {
    metalsPageFilter = new MetalsPageFilter();
    await metalsPageFilter.init();
});
