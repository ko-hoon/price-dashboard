...
function getChangeHTML(change) {
    ...
    return `<span class="${color}">${arrow} ${Math.abs(change.value).toFixed(
        2
    )} (${Math.abs(change.percent).toFixed(2)}%)</span>`;
}
...
function showLoading(elementId) {
    ...
    if (element) {
        element.innerHTML =
            '<div class="flex justify-center items-center h-64"><div class="animate-spin rounded-full h-12 w-12 border-b-2 border-purple-600"></div></div>';
    }
}
