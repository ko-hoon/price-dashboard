...
async function loadDashboardData() {
    try {
        ...
        // 환율 표시
        if (latestExchange) {
            document.getElementById('exchange-rate').textContent = formatNumber(latestExchange['환율']);
            document.getElementById('exchange-date').textContent = formatDate(latestExchange['일자']);
        }

        // 금리 표시
        if (latestExchange) {
            document.getElementById('interest-rate').textContent = latestExchange['금리'] + '%';
            document.getElementById('interest-date').textContent = formatDate(latestExchange['일자']);
        }

        // 유가 표시
        if (latestOil) {
            document.getElementById('oil-price').textContent = formatNumber(latestOil['전국']) + ' 원';
            document.getElementById('oil-date').textContent = formatDate(latestOil['일자']);
        }

        // 구리 가격 표시
        if (latestCopper) {
            document.getElementById('copper-price').textContent = formatNumber(latestCopper['구리 (달러/톤)']);
            document.getElementById('copper-date').textContent = formatDate(latestCopper['일자']);
        }

        // 최종 업데이트 시간
        const lastUpdateDate = latestExchange ? latestExchange['일자'] : new Date().toISOString().split('T')[0];
        document.getElementById('lastUpdate').textContent = `최종 업데이트: ${formatDate(lastUpdateDate)}`;
...
async function loadDashboardCharts() {
    try {
        ...
        const recentExchange = filterDataByDateRange(exchangeData, startDate, endDate, '일자');
        
        // 최근 3개월 철강 데이터
        const threeMonthsAgo = new Date();
        threeMonthsAgo.setMonth(threeMonthsAgo.getMonth() - 3);
        const recentMetal = filterDataByDateRange(metalData, threeMonthsAgo, endDate, '날짜');
...
async function loadNonMetalData() {
    try {
        ...
        const filteredData = filterDataByDateRange(data, startDate, endDate, '일자');
